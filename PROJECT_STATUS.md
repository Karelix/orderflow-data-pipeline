...
- Repo-aware manifest entries with `repo_id`, `repo_sequence`, `remote_path`,
  `data_tier`, local path, dataset type, timeframe, session type values, dates,
  rows, size, timestamps, and validation status.
...
- Manifest query script for finding which repo contains a requested dataset
  slice, defaulting to the `main` tier while still allowing `sample`, `test`,
  or all-tier lookup.
...
- Metadata-only Hugging Face upload command for publishing migrated local
  manifest and repository registry files.
...
- Bounded real end-to-end upload test to `karelix/orderflow-es-001`, including
  remote Parquet upload, local manifest update, remote metadata upload, and
  post-upload local output cleanup.
